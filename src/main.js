import "./style.css";
/* ================================================================
   SNOWBALL PORTFOLIO — v5.0 ES6 Modulaire Inline
   Architecture: Namespaces ES6, const/let, arrow functions
   Zéro bundler requis — même fichier HTML
   ================================================================ */

'use strict';

/* ════════════════════════════════════════════════════════════════
   MODULE: Config — Constantes globales
   ════════════════════════════════════════════════════════════════ */
const Config = {
  EURUSD:    1.1611,
  NAV_EUR:   25000,
  PFU:       0.314,
  TARGET_MONTHLY: 1500,
  MARKET_CACHE_TTL: 30 * 60 * 1000, // 30 min (Twelve Data Basic = 800 crédits/jour)
  YF_BASE: 'https://divkiller.michooo-45.workers.dev',
};



/* ════════════════════════════════════════════════════════════════
   MODULE: MarketData — Yahoo Finance (query1, sans clé API)
   Batch fetch, cache 5min localStorage, refresh auto
   ════════════════════════════════════════════════════════════════ */
const MarketData = (() => {
...
/* ════════════════════════════════════════════════════════════════
   MODULE: FmpData — Fondamentaux Financial Modeling Prep (24h cache)
   Champs enrichis : payout_ratio, fcf_payout, debt_ebitda,
   interest_cov, streak, div_cagr_5y, pe_cur, beta, sector, market_cap
   ════════════════════════════════════════════════════════════════ */
const FmpData = (() => {
...
/* ════════════════════════════════════════════════════════════════
   MODULE: Data — Données statiques (prices, assets, transactions)
   ════════════════════════════════════════════════════════════════ */
const Data = (() => {
...
const meta   = Data.assets; // méta pointe sur assets


/* ════════════════════════════════════════════════════════════════
   MODULE: Calc — Calculs portfolio (positions, agrégats)
   ════════════════════════════════════════════════════════════════ */
const Calc = (() => {
...
/* Globals rétrocompat — les render functions utilisent ces noms */
const toE   = Calc.toE;
const fE    = Calc.fE;
const fPct  = Calc.fPct;
const eu    = Calc.eu;
const getMV    = Calc.getMV;
const getCost  = Calc.getCost;
const getPNL   = Calc.getPNL;
const getDivA  = Calc.getDivA;
const getTotalDividends = Calc.getTotalDividends;
const getTotalTaxes     = Calc.getTotalTaxes;
const getRealizedGains  = Calc.getRealizedGains;
const getPositions      = Calc.getPositions; // alias positions[]
// raw est un getter dynamique — pointe toujours sur Calc.raw courant
Object.defineProperty(window, 'raw', { get: () => Calc.raw, configurable: true });


/* ════════════════════════════════════════════════════════════════
   MODULE: DividendSafety — Engine DSE
   ════════════════════════════════════════════════════════════════ */
...
/* Aliases rétrocompat */
const calculateDividendSafety = DividendSafety.calculate;
const dseColor  = DividendSafety.color;
const dseLabel  = DividendSafety.label;
const DSE_WEIGHTS = {payout_ratio:.25,fcf_payout:.20,debt_ebitda:.15,interest_cov:.10,div_streak:.10,div_cagr_5y:.10,earn_stability:.05,recession_res:.05};


/* ════════════════════════════════════════════════════════════════
   MODULE: Storage — IndexedDB + AES-GCM (offline-first)
   ════════════════════════════════════════════════════════════════ */
...
/* Aliases rétrocompat */
const loadImportedTx  = Storage.load;
const saveImportedTx  = Storage.save;
const clearImportedTx = Storage.clear;

/* ════════════════════════════════════════════════════════════════
   MODULE: D1Client — sync avec Cloudflare Worker / D1
   ════════════════════════════════════════════════════════════════ */
const D1Client = (() => {
...
/* ════════════════════════════════════════════════════════════════
   MODULE: BrokerImport — Parsing CSV multi-brokers
   ════════════════════════════════════════════════════════════════ */
const BrokerImport = (() => {
...
/* Aliases rétrocompat */
const processCsv           = BrokerImport.process;
const applyImportedToPortfolio = BrokerImport.applyToPortfolio;
